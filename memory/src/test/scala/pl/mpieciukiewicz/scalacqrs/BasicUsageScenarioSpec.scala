...
class BasicUsageScenarioSpec extends FeatureSpec with GivenWhenThen {
  ...
  feature("Aggregate storing and getting with event sourcing") {
    ...
    scenario("Creation and modification of user aggregate") {
      ...
      assertThat(userAggregate.aggregateRoot).isEqualTo(User("Marcin Pieciukiewicz", None))
      ...
      assertThat(userAggregate.aggregateRoot).isEqualTo(User("Marcin Pieciukiewicz", Some(Address("Warsaw", "Center", "1"))))
