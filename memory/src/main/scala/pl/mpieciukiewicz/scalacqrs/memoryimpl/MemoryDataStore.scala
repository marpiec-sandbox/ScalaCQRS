...
class MemoryDataStore(val eventStore: EventStore) extends DataStore {
  ...
  private def getAggregateWithOptionalVersion[T](aggregateRootClass: Class[T], uid: UID, version: Option[Int]): Aggregate[T] = {
    ...
    var aggregate = Aggregate(uid, 1, aggregateRoot)

    eventRows.tail.foreach((eventRow) => {
      if (eventRow.expectedAggregateVersion == aggregate.version) {
        val event = eventRow.event.asInstanceOf[ModificationEvent[T]]
        aggregate = Aggregate(aggregate.uid, aggregate.version + 1, event.apply(aggregateRoot))
      } else {
